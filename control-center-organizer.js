(() => {
  ...
  function cardText(card) {
    return normalize(`${cardTitle(card)} ${card?.textContent || ''}`);
  }
  ...
  function isToolCard(element, root) {
    ...
    if (element.matches('.control-card, .control-center-card')) return true;
    return false;
  }
  ...
  function readRecent() {
    try { return JSON.parse(localStorage.getItem(RECENT_KEY) || '[]').filter(Boolean).slice(0, 4); } catch (_) { return []; }
  }

  function saveRecent(title) {
    if (!title) return;
    try {
      const recent = [title, ...readRecent().filter((item) => item !== title)].slice(0, 4);
      localStorage.setItem(RECENT_KEY, JSON.stringify(recent));
    } catch (_) {}
  }

  function ensureStyles() {
    if (document.getElementById('control-center-organizer-style-v2')) return;
    const style = document.createElement('style');
    style.id = 'control-center-organizer-style-v2';
    style.textContent = `
      .cc-dashboard-v2{display:grid;gap:10px;margin:8px 0 14px}
      ...
      .cc-widget-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.cc-widget{position:relative;display:flex;flex-direction:column;align-items:flex-start;justify-content:space-between;gap:8px;min-height:112px;padding:13px;border:1px solid rgba(148,163,184,.25);border-radius:17px;text-align:left;box-shadow:0 5px 16px rgba(15,23,42,.055);overflow:hidden}.cc-widget::after{content:'';position:absolute;width:70px;height:70px;border-radius:50%;right:-26px;bottom:-30px;background:rgba(255,255,255,.32)}.cc-widget-icon{font-size:1.45rem}.cc-widget-title{font-size:.9rem;font-weight:900;color:#16263d}.cc-widget-desc{font-size:.69rem;line-height:1.18;color:#526275}.cc-widget-count{position:absolute;top:10px;right:10px;display:grid;place-items:center;min-width:25px;height:25px;padding:0 6px;border-radius:999px;background:rgba(255,255,255,.72);font-size:.7rem;font-weight:900;color:#334155}.cc-widget[data-tone='blue']{background:linear-gradient(145deg,#dfeeff,#edf5ff)}.cc-widget[data-tone='green']{background:linear-gradient(145deg,#def6e9,#edf9f2)}.cc-widget[data-tone='orange']{background:linear-gradient(145deg,#fff0dc,#fff7ea)}.cc-widget[data-tone='violet']{background:linear-gradient(145deg,#eee8ff,#f6f2ff)}.cc-widget[data-tone='cyan']{background:linear-gradient(145deg,#dff5fb,#ecf9fc)}.cc-widget[data-tone='red']{background:linear-gradient(145deg,#ffe7e7,#fff2f2)}.cc-widget[data-tone='slate']{background:linear-gradient(145deg,#e9eef4,#f4f7fa)}
  ...
  function groupCards(groupId) {
    return state.cards.filter((card) => card.dataset.ccGroup === groupId);
  }
  ...
  function showCards(cards, title, description) {
    ...
    state.dashboard?.scrollIntoView({ block: 'start', behavior: 'smooth' });
  }

  function openGroup(group) {
    const cards = groupCards(group.id);
    state.activeGroup = group.id;
    showCards(cards, `${group.icon} ${group.title}`, group.description);
  }
  ...
  function createDashboard() {
    ...
    dashboard.innerHTML = `<div class="cc-dashboard-head"><div><h2>Centro di controllo</h2><small>Apri una categoria o cerca una funzione</small></div></div><div class="cc-searchbar"><input type="search" autocomplete="off" placeholder="🔎 Cerca funzione…" aria-label="Cerca nel Centro di controllo"><button type="button" aria-label="Cancella ricerca">✕</button></div><div class="cc-recent" aria-label="Ultime funzioni usate"></div><div class="cc-widget-grid"></div>`;
  ...
  function createWorkspace() {
    ...
    workspace.innerHTML = `<header class="cc-workspace-head"><button class="cc-workspace-back" type="button" aria-label="Chiudi strumenti">←</button><div class="cc-workspace-heading"><strong data-cc-workspace-title>Strumenti</strong><small data-cc-workspace-subtitle></small></div></header><div class="cc-workspace-body"></div>`;
    workspace.querySelector('.cc-workspace-back').addEventListener('click', closeWorkspace);
    workspace.addEventListener('click', (event) => {
      const card = event.target.closest('details, .control-card, .control-center-card');
      if (card && state.cards.includes(card)) recordCardUse(card);
    }, true);
    return workspace;
  }
  ...
  function organize() {
    ...
    if (root.dataset.ccOrganizerInstalledV2 === '1') {
    ...
    cards.forEach((card, index) => {
      card.dataset.ccOriginalIndex = String(index);
    ...
    root.dataset.ccOrganizerInstalledV2 = '1';
    root.classList.add('control-center-organized-v2');
  ...
  window.HeraControlCenterOrganizer = {
    ...
    version: '2.0.0',
    refresh: organize,
    getState: () => ({ organized: Boolean(state.root?.dataset.ccOrganizerInstalledV2 === '1'), cards: state.cards.length, activeGroup: state.activeGroup })
  };
})();
