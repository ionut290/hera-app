...
I documenti possono contenere utente, ruolo, tipo e descrizione dell'azione, commessa, impianto, dettaglio, view, pulsante, dati tecnici dell'errore, dispositivo e data. Per le pressioni di NAVIGA e FATTO il tracciamento esistente viene ora completato con commessa, nome impianto, chiave impianto e ID SAP disponibili nel pulsante o nella scheda selezionata. La view nasconde ogni campo assente. Un ID SAP viene mostrato solo se esiste già in uno dei campi `impiantoSap`, `idSap` o `idSAP` del documento; i record storici privi di questi valori non vengono ricostruiti con dati inventati.
