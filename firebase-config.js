...
const HERA_FIRESTORE_REGISTRY_OPTIMIZER_SRC = "firestore-registry-read-optimizer.js?v=20260804a";
const HERA_REGISTRY_DEVICE_CACHE_SRC = "registry-device-cache.js?v=20260804b";

if (document.readyState === "loading") {
  // Deve essere sincrono e precedente ad app.js: intercetta soltanto le query
  // duplicate delle collezioni personale e mezzi, lasciando invariati i risultati.
  ...
  // La copia locale viene ripristinata dopo l'esecuzione degli script principali.
  document.write(`<script defer src="${HERA_REGISTRY_DEVICE_CACHE_SRC}"><\/script>`);
} else {
  if (!document.querySelector('script[data-hera-firestore-registry-optimizer="true"]')) {
  ...
  if (!document.querySelector('script[data-hera-registry-device-cache="true"]')) {
