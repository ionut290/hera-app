...
function getDisplayName(data) {
  return firstValue(data, [
    ...
    firstValue(data, ["nome", "name", "firstName"]),
    firstValue(data, ["cognome", "surname", "lastName"])
  ].filter(Boolean).join(" ") || "Operatore";
}

function getPersonnelEmail(data) {
  return normalizeEmail(firstValue(data, [
    "emailAccessoApp",
    "EMAIL_ACCESSO_APP",
    "linkedUserEmail",
    "LINKED_USER_EMAIL",
...
async function resolveAuthUser(data, email) {
  ...
  if (linkedUid) {
    try {
      return { user: await admin.auth().getUser(linkedUid), matchedBy: "linkedUserId", created: false };
    } catch (error) {
  ...
  if (email) {
    try {
      return { user: await admin.auth().getUserByEmail(email), matchedBy: "email", created: false };
    } catch (error) {
      if (error?.code !== "auth/user-not-found") throw error;
    }
  }

  return { user: null, matchedBy: "", created: false };
}
...
async function provisionPersonnelAccount(db, personnelDoc) {
  ...
  if (!user && !validEmail(personnelEmail)) {
    return {
      ...
      displayName,
      reason: "email-mancante-o-non-valida"
    };
  }
  ...
  const platformRef = db.collection("platformUsers").doc(user.uid);
...
async function main() {
  ...
  if (summary.error > 0) {
    throw new Error(`Provisioning terminato con ${summary.error} errori.`);
  }
}
