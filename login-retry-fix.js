(function installLoginRetryFix() {
  ...
  const OPERATOR_USERNAME_LOGIN_URL = "https://europe-west1-hera-app-6cd2b.cloudfunctions.net/registerTester";
  ...
  function friendlyLoginError(error) {
    ...
    if (code === "auth/email-not-verified") return "Email non ancora verificata. Apri il messaggio ricevuto da Firebase e conferma l’indirizzo.";
  ...
  async function createSelfRegisteredProfile(user, details) {
    ...
    await firebase.firestore().collection("platformUsers").doc(user.uid).set({
      uid: user.uid, email: details.email, displayName,
      nome: details.firstName, cognome: details.lastName, nomeCompleto: displayName,
      firstName: details.firstName, lastName: details.lastName,
      teamId: "", role: "user", ruolo: "user", isAdmin: false, admin: false,
      statoAccount: "in_attesa", accountStatus: "in_attesa",
      primoAccessoAt: firebase.firestore.FieldValue.serverTimestamp(),
      firstLoginAt: firebase.firestore.FieldValue.serverTimestamp(),
      approvatoAt: null, approvedAt: null, approvatoDa: null, approvedBy: null,
      numeroRichieste: 0, requestCount: 0, permissions: {}, mustChangePassword: false,
      selfRegistered: true, authProviders: ["password"],
  ...
  function openRegistrationDialog(email, password) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const cancel = () => { cleanup(); closeRegistrationDialog(); reject(new Error("Creazione account annullata.")); };
      const preventDialogCancel = (event) => { event.preventDefault(); cancel(); };
      const submit = async (event) => {
        ...
        if (!firstName || !lastName) { elements.feedback.textContent = "Inserisci nome e cognome."; return; }
        if (chosenPassword.length < MIN_REGISTRATION_PASSWORD_LENGTH) { elements.feedback.textContent = `La password deve contenere almeno ${MIN_REGISTRATION_PASSWORD_LENGTH} caratteri.`; return; }
        if (chosenPassword !== confirmation) { elements.feedback.textContent = "Le due password non coincidono."; return; }
        registrationPending = false;
        ...
        try {
          ...
          cleanup(); closeRegistrationDialog(); resolve({ verificationRequired: true });
        } catch (error) {
          registrationPending = true;
  ...
  async function startRegistrationFromLogin() {
    ...
    } finally { if (createButton) createButton.disabled = false; }
  }
  ...
  async function signInWithOperatorUsername(auth, username, password) {
    ...
    try {
      response = await fetch(OPERATOR_USERNAME_LOGIN_URL, {
        ...
        body: JSON.stringify({ data: { action: "loginUsername", username, password } })
      });
    } catch (networkError) {
    ...
    if (!response.ok || !token) {
      ...
      error.code = status === "RESOURCE_EXHAUSTED" ? "auth/too-many-requests" : (status === "UNAVAILABLE" ? "auth/network-request-failed" : "auth/invalid-credential");
      throw error;
    }
  ...
  async function handleLogin(event) {
    ...
    const password = String(passwordInput?.value || "");
    if ((!identifier.email && !identifier.username) || !password) { if (feedback) feedback.textContent = "Inserisci username/email e password."; return; }
    ...
    try {
      ...
      try {
        ...
        if (credential.user && credential.user.emailVerified === false) {
          await auth.signOut();
          const verificationError = new Error("Email non verificata."); verificationError.code = "auth/email-not-verified"; throw verificationError;
        }
      } catch (loginError) {
      ...
      if (passwordInput) { passwordInput.value = ""; passwordInput.focus({ preventScroll: true }); }
      window.scrollTo({ left: 0, top: 0, behavior: "auto" });
    } finally {
      if (loginButton) { loginButton.disabled = false; loginButton.textContent = "Entra"; }
    }
  }
  ...
  function configureUsernameLoginField() {
    ...
    if (!input) return;
    input.type = "text"; input.inputMode = "email";
    ...
    if (gateMessage && /email e password/i.test(gateMessage.textContent || "")) gateMessage.textContent = (gateMessage.textContent || "").replace(/email e password/i, "username/email e password");
  }
