(function installPasswordAccessManager() {
  ...
  function formatError(error) {
    ...
    if (code === "auth/invalid-credential") return "Credenziali non valide. Riprova dopo essere rientrato con Google.";
  ...
  function createPanel() {
    ...
    panel.querySelector("#password-access-close-btn")?.addEventListener("click", () => {
  ...
  function updatePanelState() {
    ...
    status.textContent = googleLinked
      ? `Account Google rilevato: ${user.email || "email non disponibile"}. Imposta una password mantenendo lo stesso account e gli stessi dati.`
      : `Imposta una password per ${user.email || "questo account"}.`;
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      await user.reload();
      feedback.textContent = "✅ Accesso con password attivato. Da ora puoi entrare con Google oppure con email e password mantenendo lo stesso account.";
