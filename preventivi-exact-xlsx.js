(()=>{'use strict';
const M=window.HeraPreventiviModels,P=M?.PV;if(!M||!P)return;
const NS='http://schemas.openxmlformats.org/spreadsheetml/2006/main',RNS='http://schemas.openxmlformats.org/officeDocument/2006/relationships',PRNS='http://schemas.openxmlformats.org/package/2006/relationships',XNS='http://www.w3.org/XML/1998/namespace';
const parser=new DOMParser(),serializer=new XMLSerializer(),parse=text=>{const d=parser.parseFromString(text,'application/xml');if(d.querySelector('parsererror'))throw Error('Il modello Excel contiene XML non leggibile.');return d};
const norm=v=>String(v??'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9%]+/g,' ').trim();
const colN=s=>[...String(s||'').toUpperCase()].reduce((n,c)=>n*26+c.charCodeAt(0)-64,0),colL=n=>{let o='';for(let x=n;x>0;x=Math.floor((x-1)/26))o=String.fromCharCode(65+(x-1)%26)+o;return o};
const pos=r=>{const m=String(r||'').match(/^([A-Z]+)(\d+)$/i);return m?{col:colN(m[1]),row:Number(m[2])}:null},ref=(c,r)=>`${colL(c)}${r}`;
const child=(n,k)=>[...n.children].find(x=>x.localName===k)||null;
const text=(cell,shared)=>{if(!cell)return'';const t=cell.getAttribute('t');if(t==='inlineStr')return[...cell.getElementsByTagNameNS(NS,'t')].map(x=>x.textContent||'').join('');const v=child(cell,'v')?.textContent??'';return t==='s'?(shared[Number(v)]??''):v};
const shared=zip=>{const e=zip.file('xl/sharedStrings.xml');if(!e)return[];const d=parse(e.asText());return[...d.getElementsByTagNameNS(NS,'si')].map(si=>[...si.getElementsByTagNameNS(NS,'t')].map(t=>t.textContent||'').join(''))};
const sheetPath=zip=>{const w=parse(zip.file('xl/workbook.xml').asText()),rs=parse(zip.file('xl/_rels/workbook.xml.rels').asText()),s=w.getElementsByTagNameNS(NS,'sheet')[0];if(!s)throw Error('Il modello Excel non contiene fogli.');const id=s.getAttributeNS(RNS,'id')||s.getAttribute('r:id'),rel=[...rs.getElementsByTagNameNS(PRNS,'Relationship')].find(x=>x.getAttribute('Id')===id);if(!rel)throw Error('Il foglio principale non è collegato.');const target=rel.getAttribute('Target').replace(/^\/+/, '');return target.startsWith('xl/')?target:`xl/${target}`};
const merges=d=>[...d.getElementsByTagNameNS(NS,'mergeCell')].map(x=>{const [a,b]=x.getAttribute('ref').split(':'),s=pos(a),e=pos(b||a);return s&&e?{s,e}:null}).filter(Boolean),mergeAt=(list,r)=>{const p=pos(r);return p?list.find(m=>p.row>=m.s.row&&p.row<=m.e.row&&p.col>=m.s.col&&p.col<=m.e.col):null};
const data=d=>d.getElementsByTagNameNS(NS,'sheetData')[0],row=(d,n,create=false)=>{let r=[...d.getElementsByTagNameNS(NS,'row')].find(x=>Number(x.getAttribute('r'))===n);if(r||!create)return r;r=d.createElementNS(NS,'row');r.setAttribute('r',String(n));const sd=data(d),next=[...sd.children].find(x=>Number(x.getAttribute('r'))>n);sd.insertBefore(r,next||null);return r};
const cell=(d,r,create=false)=>{let c=[...d.getElementsByTagNameNS(NS,'c')].find(x=>x.getAttribute('r')===r);if(c||!create)return c;const p=pos(r),rw=row(d,p.row,true);c=d.createElementNS(NS,'c');c.setAttribute('r',r);const next=[...rw.children].find(x=>colN((x.getAttribute('r')||'').replace(/\d+/g,''))>p.col);rw.insertBefore(c,next||null);return c};
const clear=c=>{[...c.children].forEach(x=>c.removeChild(x));c.removeAttribute('t')},inline=(d,r,v)=>{const c=cell(d,r,true),s=c.getAttribute('s');clear(c);if(s!==null)c.setAttribute('s',s);c.setAttribute('t','inlineStr');const i=d.createElementNS(NS,'is'),t=d.createElementNS(NS,'t');t.setAttributeNS(XNS,'xml:space','preserve');t.textContent=String(v??'');i.appendChild(t);c.appendChild(i)},number=(d,r,v)=>{const c=cell(d,r,true),s=c.getAttribute('s');clear(c);if(s!==null)c.setAttribute('s',s);const x=d.createElementNS(NS,'v');x.textContent=Number.isFinite(Number(v))?String(Number(v)):'0';c.appendChild(x)},formula=(d,r,f,cached=0)=>{const c=cell(d,r,true),s=c.getAttribute('s');clear(c);if(s!==null)c.setAttribute('s',s);const x=d.createElementNS(NS,'f'),v=d.createElementNS(NS,'v');x.textContent=f;v.textContent=String(Number(cached)||0);c.append(x,v)};
