...
const loadScript=(marker,src,error)=>new Promise(resolve=>{
  ...
  const script=document.createElement('script');script.src=src;script.async=false;script.setAttribute(marker,'1');script.addEventListener('load',resolve,{once:true});script.addEventListener('error',()=>{console.warn(error);resolve();},{once:true});document.head.appendChild(script);
});
