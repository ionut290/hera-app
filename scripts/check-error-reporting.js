...
assert.match(loader, /app-error-monitor\.js\?v=20260829-firestore-quota-dedup1/);
...
assert.match(monitor, /FIRESTORE_STORAGE_QUOTA_FINGERPRINT\s*=\s*"firestore-local-storage-quota"/);
assert.match(monitor, /firestore_\(\?:clients\|targets\|mutations\)_firestore/);
assert.match(centerBackend, /FIRESTORE_STORAGE_QUOTA_FINGERPRINT\s*=\s*"firestore-local-storage-quota"/);
assert.match(centerBackend, /!report\.manual\s*&&\s*isFirestoreStorageQuotaError/);
