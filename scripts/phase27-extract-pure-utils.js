...
const FORBIDDEN_NAMES = /(?:fatto|whazz|whats|firebase|firestore|auth|login|squad|commess|impiant|ore|gps|map|weather|meteo|offline|sync|admin|user|photo|foto|document|window|ui|operator|position|corso|worklimate)/i;
...
function isPureCandidate(fn, source) {
  ...
  if (text.length < 40 || text.length > 1600) return false;
  ...
  walk(fn.body, (node, parent) => {
    if (!safe) return;
    if (node.type === "ThisExpression" || node.type === "AwaitExpression" || node.type === "YieldExpression" || node.type === "NewExpression") { safe = false; return; }
...
const extra = ["", "// Phase 27: pure utilities extracted from app.js", "(function exposeMoreVargaPureUtils(global) {", "  const api = {};"];
...
console.log(`Fase 27: estratte ${names.length} utility pure: ${names.join(", ")}`);
