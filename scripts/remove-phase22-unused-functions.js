...
const TARGETS = {
  "active-commesse-first-boot-guard.js": ["parseSnapshotObserver"],
  "app.js": ["getHoursEntrySquadraIndexes", "openWorklimateDetails"]
};
...
for (const [relativePath, names] of Object.entries(TARGETS)) {
  ...
  for (const name of names) {
    ...
    if (matches.length !== 1) throw new Error(`${relativePath}: ${name} dichiarazioni=${matches.length}`);
    const refs = source.match(new RegExp(`\\b${name}\\b`, "g")) || [];
    if (refs.length !== 1) throw new Error(`${relativePath}: ${name} riferimenti=${refs.length}; rimozione annullata`);
...
console.log("Fase 22: rimosse 3 funzioni indipendenti inutilizzate.");
